...
export function render() {
  ...
  // non vanno mostrate come "alta priorita'" ogni giorno. In piu': una
  ...
  container.innerHTML = `
    <table class="desktop-table">
      <tr><td>Riserva di LiquiditÃ  (Saldo Cassa / Spese Fisse Mensili)</td><td class="amount ${months == null ? "" : months >= 3 ? "text-green" : "text-red"}" style="text-align:right">${months == null ? "n/d" : months.toFixed(1) + " Mesi"}</td></tr>
    ...
    <div class="mobile-cards-container">
      <div class="m-card"><div class="m-card-header"><span class="m-card-title">Riserva di LiquiditÃ </span><span class="m-card-amount ${months == null ? "" : months >= 3 ? "text-green" : "text-red"}">${months == null ? "n/d" : months.toFixed(1) + " Mesi"}</span></div></div>
    ...
    ${critical.length ? `
      <h3 style="font-size:0.85rem;margin:16px 0 8px;color:var(--text-muted)">Scadenze ad alta prioritÃ  non completate</h3>
